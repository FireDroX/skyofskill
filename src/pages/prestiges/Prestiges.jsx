...
const Prestiges = () => {
    ...
    const handleBtn = (e) => {
        switch(e) {
            case "+": if(page !== 4) setPage(page + 1); break;
            case "-": if(page !== 0) setPage(page - 1); break;
            default: break;
        }
    }
    ...
    const calcPrice = () => {
        ...
        const totalPrice = selectedPrestiges.reduce((total, prestige) => total + prestige.prix, 0);
        setPrice(totalPrice);
    }
            ...
            <div>
                <div className="minage-prestige">
                    <div className="input-prestige">
                        ...
                        <small>Prix de {newPrestige[page][0].name} à {newPrestige[page][16].name}</small>
                    </div>
                    <div className="calc-prestige">
                        <div>
                            <h6>Votre prestige</h6>
                            <select defaultValue={currentPrestige} onChange={(e) => setCurrentPrestige(e.target.value)}>
                                {prestiges.map((p, index) => (
                                    <option key={index} value={index + 1}>{p.name}</option>
                                ))}
                            </select>
                        </div>
                        <div>
                            <h6>Prestige souhaité</h6>
                            <select defaultValue={nextPrestige} onChange={(e) => setNextPrestige(e.target.value)}>
                                {prestiges.map((p, index) => (
                                    <option key={index} value={index + 1}>{p.name}</option>
                                ))}
                            </select>
                        ...
                        <div>
                            ...
                            <small>( {formatNumberWithSpaces(price)}) <FaCoins /></small>
                        </div>
                    </div>
                </div>
            </div>
